...
def _score_synthesizer_on_dataset(name, synthesizer, iteration, args):
    try:
        ...
        save_dir = os.path.join("results", args.dataset_name)
        os.makedirs(save_dir, exist_ok=True)

        save_path = os.path.join(save_dir, f"syn.csv")
        df = pd.DataFrame(synthesized)
        df.to_csv(save_path, index=False)
        
        npz_path = os.path.join(save_dir, f"syn.npz")
        np.savez_compressed(npz_path, synthesized=synthesized)
        
        LOGGER.info(f"Synthesized data saved at: {save_path}")
